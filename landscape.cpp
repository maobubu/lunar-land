...

{
  ...
  float s = LANDSCAPE_WIDTH / (max.x - min.x);//scale the landscape

  mat4 modelToWorldTransform = scale( s, -s, 1 ) * translate( -min.x, -max.y, 0 );
  //Y axis swapped, so have to -maxY
...

{
  ...
  // range.
	vec3 v = segHead - segTail;//segment vector
	vec3 u = position - segTail;
	vec3 perpen;
...
vec3 Landscape::segmentmiddle(vec3 segTail,vec3 segHead) {
	vec3 middle = (0.5)*(segHead+segTail);
	return middle;

}
float Landscape::altitude(vec3 position)
{
	float minAltitude;
	float minmiddle=MAXFLOAT;
	for (int i = 0; i<numVerts - 1; i++) {

		vec3 middle_point = segmentmiddle(
			vec3(landscapeVerts[2 * i], landscapeVerts[2 * i + 1], 0),
			vec3(landscapeVerts[2 * (i + 1)], landscapeVerts[2 * (i + 1) + 1], 0));
		float middle_length = abs(position.x - middle_point.x);
//calculate the length between each segments middle point and the lander, pick the smallest
//so that segment will be the one the lander is above, calculate the distance.
		if (middle_length < minmiddle) {
			minmiddle = middle_length;
			minAltitude = sqrt((middle_point - position) * (middle_point - position));
			//if smaller than the previous middle length, than calculate the distance,
			//orelse not.
		}
	}
	return minAltitude;
...
vec3 Landscape::findClosestPoint( vec3 position )//returns the space ships position
